...
# -------------------------------------------------------------------------------------------------------
OPEN1="MSX SDCC Make Script Copyright © 2020-2021 Danilo Angelo, 2021 Pedro Medeiros, 2021 Paolo Oliveira"
OPEN2="version 00.05.00 - Codename Mac\'n\'Tux"
# -------------------------------------------------------------------------------------------------------

if [ $# -lt 2 ]
then
  echo "MSX SDCC Make Script"
  echo "[-] ERROR! Syntaxt: $0 <profile=[Debug|Release]> <type=[BIN|ROM|MSXDOS]>"
  exit 1
fi
...
configure_target() {
    ...
    if [[ ! -f "$CFGPATH/TargetConfig_$PROFILE.txt" ]]; then
        debug $DBG_STEPS File $CFGPATH/TargetConfig_$PROFILE.txt not found.
        exit 1
    fi
    ...
    done < "$CFGPATH/TargetConfig_$PROFILE.txt"
...
application_settings() {
    ...
    done < $CFGPATH/ApplicationSettings.txt
...
collect_include_dirs() {
    ...
    while read -r INCDIR; do
        if [[ -n $INCDIR && ${INCDIR:0:1} != ';' ]]; then
            ...
            INCDIR=$(path_replace "$INCDIR" '[CURRENT_DIR]' "$CURRENT_DIR")
            INCDIRS[$I]="-I'$INCDIR'"
            I+=1
            debug $DBG_DETAIL "Collected $INCDIR"
        fi
    done < CFG/IncludeDirectories.txt
...
build_lib() {
    ...
    while read -r LIBFILE; do
        if [[ -n $LIBFILE && ${LIBFILE:0:1} != ';' ]]; then
            ...
            LIBFILE=$(path_replace "$LIBFILE" '[CURRENT_DIR]' "$CURRENT_DIR")
...
compile () {
    ...
    while read -r APPFILE; do
        if [[ -n $APPFILE && ${APPFILE:0:1} != ';' ]]; then
            ...
            APPFILE=$(path_replace "$APPFILE" '[CURRENT_DIR]' "$CURRENT_DIR")
    ...
    done < $CFGPATH/ApplicationSources.txt
...
if [[ -z $3 ]]; then    # no parameters specified
    compile
    build_msx_bin
    finish
fi
